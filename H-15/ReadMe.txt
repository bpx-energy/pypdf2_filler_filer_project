...
All the code lives in pdfProcessor.py The only variables that need amending are:
...
The dictionary is now output to the fields variable. There is a good chance this is not viewable in your locals due to it's large size. Type this variable name into the console to see the output if this is the case- and decipher the field names. The field names in the example file that comes with this directory (PatientIntakeForm.pdf) will output the below. The field names visible in the below snippet are Address[0] and Age[0] and CellNum[0] These are what will need to be passed as keys into the variable field_dictionary to write to these fields.
